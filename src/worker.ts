...
interface AssetDescriptor {
  ...
  symbol: string;
  name: string;
  assetClass: "crypto" | "stock" | "etf" | "fund";
  provider: string;
...
const FREE_FUNDS = [["VFIAX", "VFIAX", "Vanguard 500 Index Fund Admiral"], ["VTSAX", "VTSAX", "Vanguard Total Stock Market Index Fund"], ["VTWAX", "VTWAX", "Vanguard Total World Stock Index Fund"], ["VBTLX", "VBTLX", "Vanguard Total Bond Market Index Fund"], ["VGSLX", "VGSLX", "Vanguard Real Estate Index Fund"], ["VIGAX", "VIGAX", "Vanguard Growth Index Fund"], ["VSMAX", "VSMAX", "Vanguard Small-Cap Index Fund"], ["VTIAX", "VTIAX", "Vanguard Total International Stock Index Fund"], ["FXAIX", "FXAIX", "Fidelity 500 Index Fund"], ["FZROX", "FZROX", "Fidelity ZERO Total Market Index Fund"], ["FCNTX", "FCNTX", "Fidelity Contrafund"], ["FSKAX", "FSKAX", "Fidelity Total Market Index Fund"], ["SWPPX", "SWPPX", "Schwab S&P 500 Index Fund"], ["SWTSX", "SWTSX", "Schwab Total Stock Market Index Fund"], ["PRWCX", "PRWCX", "T. Rowe Price Capital Appreciation Fund"], ["TRBCX", "TRBCX", "T. Rowe Price Blue Chip Growth Fund"]] as const;
const ASSETS: AssetDescriptor[] = [
  ...
  ...makeAssets(FREE_ETFS, "etf", "Yahoo Finance 公共日/周/月线 · Nasdaq 日线备源", "public"),
  ...makeAssets(FREE_FUNDS, "fund", "Yahoo Finance 公共日/周/月线", "public")
];
...
type ProviderRoute = { providerChain: string[]; providerUsed: string; failedSources: Array<{ provider: string; reason: string }> };
type CandlePayload = { instrument: AssetDescriptor; interval: string; source: string; volumeAvailable: true; cachedAt: string; sample: SampleMetadata; route?: ProviderRoute; candles: Candle[] };
...
async function cryptoCandles(request: Request, env: Env, symbol: string, interval: string, limit: number): Promise<Response> {
  ...
  try {
    ...
    const route: ProviderRoute = { providerChain: ["Kraken OHLC", "Coinbase Exchange"], providerUsed: "Kraken OHLC", failedSources: [] }; const response: CandlePayload = { instrument: asset, interval, source: `Kraken OHLC · ${INTERVAL_LABELS[interval]} · 仅已收盘K线（5 分钟 Cloudflare 缓存）`, volumeAvailable: true, cachedAt: fetchedAt, sample, route, candles };
...
type NasdaqPayload = { data?: { tradesTable?: { rows?: Array<{ date?: string; close?: string; open?: string; high?: string; low?: string; volume?: string }> } } };
...
const numericText = (value: string | undefined) => Number((value ?? "").replace(/[$,]/g, ""));
function parseNasdaqCandles(payload: NasdaqPayload): Candle[] { return (payload.data?.tradesTable?.rows ?? []).map(row => ({ time: Math.floor(new Date(`${row.date ?? ""}T00:00:00Z`).getTime() / 1000), open: numericText(row.open), high: numericText(row.high), low: numericText(row.low), close: numericText(row.close), volume: numericText(row.volume) })).filter(candle => [candle.time, candle.open, candle.high, candle.low, candle.close].every(Number.isFinite)).sort((a, b) => a.time - b.time); }
function sampleFor(candles: Candle[], interval: string, requested: number): SampleMetadata { const fetchedAt = new Date().toISOString(); const gapCount = countGaps(candles, interval); return { dataPolicy: "closed_candles_only", providerMaxCandles: 0, requestedCandles: requested, returnedCandles: candles.length, startTime: new Date(candles[0].time * 1000).toISOString(), endTime: new Date(candles.at(-1)!.time * 1000).toISOString(), fetchedAt, excludedCurrentCandle: true, gapCount, continuous: gapCount === 0 }; }
function marketResponse(asset: AssetDescriptor, interval: string, candles: Candle[], used: string, route: ProviderRoute): CandlePayload { const sample = sampleFor(candles, interval, candles.length); return { instrument: asset, interval, source: `${used} · ${INTERVAL_LABELS[interval]} · 仅已收盘K线（5 分钟 Cloudflare 缓存）`, volumeAvailable: true, cachedAt: sample.fetchedAt, sample, route, candles }; }
async function yahooSeries(asset: AssetDescriptor, interval: string, limit: number): Promise<Candle[]> { const yahooInterval = interval === "1w" ? "1wk" : interval === "1M" ? "1mo" : "1d"; const endpoint = new URL(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(asset.symbol)}`); endpoint.searchParams.set("range", interval === "1d" ? "10y" : "max"); endpoint.searchParams.set("interval", yahooInterval); endpoint.searchParams.set("events", "history"); const upstream = await fetch(endpoint.toString(), { headers: { Accept: "application/json", "User-Agent": "Mozilla/5.0 QuantWatchResearch/1.0" }, cf: { cacheTtl: CANDLE_CACHE_TTL_SECONDS, cacheEverything: true } }); if (!upstream.ok) throw new Error(`HTTP ${upstream.status}`); const candles = parseYahooCandles(await upstream.json() as YahooPayload).slice(0, -1).slice(-Math.min(Math.max(limit, 60), 2000)); if (candles.length < 60) throw new Error(`有效样本仅 ${candles.length} 根`); return candles; }
async function nasdaqSeries(asset: AssetDescriptor, limit: number): Promise<Candle[]> { const endpoint = new URL(`https://api.nasdaq.com/api/quote/${encodeURIComponent(asset.symbol)}/historical`); endpoint.searchParams.set("assetclass", asset.assetClass === "etf" ? "etf" : "stocks"); endpoint.searchParams.set("fromdate", new Date(Date.now() - 1000 * 60 * 60 * 24 * 365 * 10).toISOString().slice(0, 10)); endpoint.searchParams.set("todate", new Date().toISOString().slice(0, 10)); endpoint.searchParams.set("limit", String(Math.min(Math.max(limit, 60), 5000))); const upstream = await fetch(endpoint.toString(), { headers: { Accept: "application/json, text/plain, */*", "User-Agent": "Mozilla/5.0 QuantWatchResearch/1.0" }, cf: { cacheTtl: CANDLE_CACHE_TTL_SECONDS, cacheEverything: true } }); if (!upstream.ok) throw new Error(`HTTP ${upstream.status}`); const candles = parseNasdaqCandles(await upstream.json() as NasdaqPayload).slice(-Math.min(Math.max(limit, 60), 2000)); if (candles.length < 60) throw new Error(`有效样本仅 ${candles.length} 根`); return candles; }
async function multiSourceMarketCandles(request: Request, env: Env, asset: AssetDescriptor, interval: string, limit: number): Promise<Response> { if (!new Set(["1d", "1w", "1M"]).has(interval)) return noStoreJson({ error: "interval_not_supported", supported: ["1d", "1w", "1M"], message: "免费股票、ETF 与基金研究数据当前提供日线、周线和月线；分钟线需使用专业数据源。" }, request, env, 400); const key = candleCacheKey(asset.id, interval); const cached = await env.SNAPSHOT_CACHE.get(key); if (cached) { try { return noStoreJson(JSON.parse(cached), request, env, 200); } catch { await env.SNAPSHOT_CACHE.delete(key); } } const route: ProviderRoute = { providerChain: asset.assetClass === "fund" ? ["Yahoo Finance"] : ["Yahoo Finance", "Nasdaq Historical"], providerUsed: "", failedSources: [] }; try { const candles = await yahooSeries(asset, interval, limit); route.providerUsed = "Yahoo Finance"; const payload = marketResponse(asset, interval, candles, "Yahoo Finance 公共研究数据", route); await env.SNAPSHOT_CACHE.put(key, JSON.stringify(payload), { expirationTtl: CANDLE_CACHE_TTL_SECONDS }); return noStoreJson(payload, request, env, 200); } catch (error) { route.failedSources.push({ provider: "Yahoo Finance", reason: error instanceof Error ? error.message : "unknown_error" }); } if (asset.assetClass !== "fund" && interval === "1d") { try { const candles = await nasdaqSeries(asset, limit); route.providerUsed = "Nasdaq Historical"; const payload = marketResponse(asset, interval, candles, "Nasdaq 历史日线备源", route); await env.SNAPSHOT_CACHE.put(key, JSON.stringify(payload), { expirationTtl: CANDLE_CACHE_TTL_SECONDS }); return noStoreJson(payload, request, env, 200); } catch (error) { route.failedSources.push({ provider: "Nasdaq Historical", reason: error instanceof Error ? error.message : "unknown_error" }); } } return noStoreJson({ error: "all_public_sources_failed", message: "所有匹配的公开数据源均未返回足够的已收盘K线。", route }, request, env, 502); }
function coinbaseProduct(symbol: string): string | null { const clean = symbol.replace(/^KRAKEN:/, "").replace(/USDT$/, "USD").replace(/^XBT/, "BTC"); const match = clean.match(/^([A-Z0-9]+)USD$/); return match ? `${match[1]}-USD` : null; }
function parseCoinbaseCandles(payload: unknown): Candle[] { if (!Array.isArray(payload)) return []; return payload.map(row => Array.isArray(row) && row.length >= 6 ? { time: Number(row[0]), low: Number(row[1]), high: Number(row[2]), open: Number(row[3]), close: Number(row[4]), volume: Number(row[5]) } : null).filter((row): row is Candle => row != null && [row.time, row.open, row.high, row.low, row.close, row.volume].every(Number.isFinite)).sort((a, b) => a.time - b.time); }
function aggregateCoinbaseFourHours(candles: Candle[]): Candle[] { const buckets = new Map<number, Candle[]>(); for (const candle of candles) { const key = Math.floor(candle.time / 14_400) * 14_400; const group = buckets.get(key) ?? []; group.push(candle); buckets.set(key, group); } return [...buckets.entries()].sort(([left], [right]) => left - right).map(([time, group]) => ({ time, open: group[0].open, high: Math.max(...group.map(candle => candle.high)), low: Math.min(...group.map(candle => candle.low)), close: group.at(-1)!.close, volume: group.reduce((sum, candle) => sum + candle.volume, 0) })); }
async function coinbaseFallback(request: Request, env: Env, symbol: string, interval: string, limit: number, krakenFailure: string): Promise<Response> { const granularity: Record<string, number> = { "1m": 60, "5m": 300, "15m": 900, "1h": 3600, "4h": 3600, "1d": 86400 }; const product = coinbaseProduct(symbol); if (!product || !granularity[interval]) return noStoreJson({ error: "all_public_sources_failed", message: "Kraken 未返回可用数据，且该周期没有匹配的 Coinbase 公开备源。", route: { providerChain: ["Kraken OHLC", "Coinbase Exchange"], providerUsed: "", failedSources: [{ provider: "Kraken OHLC", reason: krakenFailure }] } }, request, env, 502); try { const endpoint = new URL(`https://api.exchange.coinbase.com/products/${product}/candles`); endpoint.searchParams.set("granularity", String(granularity[interval])); const upstream = await fetch(endpoint.toString(), { headers: { Accept: "application/json", "User-Agent": "Mozilla/5.0 QuantWatchResearch/1.0" }, cf: { cacheTtl: CANDLE_CACHE_TTL_SECONDS, cacheEverything: true } }); if (!upstream.ok) throw new Error(`HTTP ${upstream.status}`); const rawCandles = parseCoinbaseCandles(await upstream.json()).slice(0, -1); const candles = (interval === "4h" ? aggregateCoinbaseFourHours(rawCandles).slice(0, -1) : rawCandles).slice(-Math.min(Math.max(limit, 60), 299)); if (candles.length < 60) throw new Error(`有效样本仅 ${candles.length} 根`); const asset = ASSETS.find(candidate => candidate.id === symbol) ?? { id: symbol, symbol: product.replace("-", "/"), name: `${product} · Coinbase`, assetClass: "crypto" as const, provider: "Coinbase Exchange", availability: "public" as const }; const route: ProviderRoute = { providerChain: ["Kraken OHLC", "Coinbase Exchange"], providerUsed: "Coinbase Exchange", failedSources: [{ provider: "Kraken OHLC", reason: krakenFailure }] }; const payload = marketResponse(asset, interval, candles, "Coinbase Exchange 公开备源", route); await env.SNAPSHOT_CACHE.put(`${candleCacheKey(symbol, interval)}:coinbase`, JSON.stringify(payload), { expirationTtl: CANDLE_CACHE_TTL_SECONDS }); return noStoreJson(payload, request, env, 200); } catch (error) { return noStoreJson({ error: "all_public_sources_failed", message: "Kraken 与 Coinbase 公开数据源均未返回足够的已收盘K线。", route: { providerChain: ["Kraken OHLC", "Coinbase Exchange"], providerUsed: "", failedSources: [{ provider: "Kraken OHLC", reason: krakenFailure }, { provider: "Coinbase Exchange", reason: error instanceof Error ? error.message : "unknown_error" }] } }, request, env, 502); } }
async function publicCandles(request: Request, env: Env, symbol: string, interval: string, limit: number, sourceMode = "auto"): Promise<Response> { const asset = ASSETS.find(candidate => candidate.id === symbol); if (asset && asset.assetClass !== "crypto") return multiSourceMarketCandles(request, env, asset, interval, limit); if (sourceMode === "coinbase") return coinbaseFallback(request, env, symbol, interval, limit, "研究者手动选择 Coinbase 备源"); const kraken = await cryptoCandles(request, env, symbol, interval, limit); if (kraken.ok) return kraken; const body = await kraken.clone().json().catch(() => ({})) as { message?: string; error?: string }; return coinbaseFallback(request, env, symbol, interval, limit, body.message ?? body.error ?? "Kraken 返回不可用响应"); }
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (request.method === "GET") {
        ...
        if (url.pathname === "/api/candles") return publicCandles(request, env, url.searchParams.get("symbol")?.toUpperCase() ?? "", url.searchParams.get("interval") ?? "4h", Number(url.searchParams.get("limit") ?? 350), url.searchParams.get("source") ?? "auto");
